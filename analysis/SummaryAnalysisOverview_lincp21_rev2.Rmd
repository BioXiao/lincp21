...
cuff<-readCufflinks(dir=all,gtfFile="/n/rinn_data2/users/agroff/Brainmap/annotation/mm10_gencode_vM2_with_lncRNAs_and_LacZ.gtf",genome=genome)
...
#pdf("dendrograms.pdf")
csDendro(genes(cuff))
#'dendrogram' with 2 branches and 16 members total, at height  0.3757501 
...
#setwd("/Volumes/valor2/users/agroff/seq/OtherMice/Diana/lincP21/analysis/")
...
myLengths<-myLengths[!grepl("Un",names(myLengths))] #ALSO ADDED 3/11/16
...
#fullTable<-getTable(cuff)
#save(fullTable,file="fullTable_contrastSheet.Rdata")
load("fullTable_contrastSheet.Rdata")
shortFullTable<-fullTable[,c("start","end","chromosome","gene_short_name",grep("significant",names(fullTable),value=TRUE))]
strain<-"Trp53cor1"

for (i in 5:10){ #shortFulltable sigs start at 5..10
  print(i)
  myGene<-fullTable[which(fullTable$gene_short_name==strain),][1,] #any problems w this?
  chromosome<-myGene$chromosome
  length<-abs(myGene$end-myGene$start)
  start<-myGene$start-((windowSize/2)-length/2)
  end<-myGene$end+((windowSize/2)-length/2)

  #exclude lincRNA in significance calc...
  #CHANGE HERE FOR CONTRAST SHEET CALC!! 
  
  sigGenesRegion<-shortFullTable[which(shortFullTable[,3]==chromosome & shortFullTable[,1]>=start & shortFullTable[,2]<=end & shortFullTable[,4]!=strain & shortFullTable[,i]=="yes"),]
  nSig<-nrow(ddply(sigGenesRegion,.(gene_short_name),head,n=1))
  ...
  for (j in 1:nIter){
    write(j,stderr())
    sigGenesRegion<-shortFullTable[which(shortFullTable[,3]==seqnames(myRandom[j])@values & shortFullTable[,i]=="yes" & shortFullTable[,1]>=start(myRandom[j])-(windowSize/2) & shortFullTable[,2]<=end(myRandom[j])+(windowSize/2)),]
    nSigIter<-nrow(ddply(sigGenesRegion,.(gene_short_name),head,n=1))
  ...
  tissue<-str_split_fixed(names(shortFullTable)[i],"_",3)[2]
  genesInRegion<-fullTable[which(fullTable[,80]==chromosome & fullTable[,9]>=start & fullTable[,10]<=end),]
  genesInRegion<-genesInRegion[,c("chromosome","start","end","gene_short_name",grep(tissue,names(genesInRegion),value=TRUE))]
  genesInRegion<-genesInRegion[which(genesInRegion[,5]=="OK"),] #ADDED THIS 3/10/16
  genesInRegion<-genesInRegion[which(genesInRegion$gene_short_name!="Tug1"),]
  genesInRegion$start<-myGene$start-genesInRegion$start
  
  
  colnames(genesInRegion)[8]<-"log2foldchange"
  colnames(genesInRegion)[9]<-"test_stat"
  data<-ddply(genesInRegion,.(gene_short_name),head,n=1)
  data$test_stat<-as.numeric(data[,9])
  data$log2foldchange<-as.numeric(data[,8])
  data$was_na<-"no"
  ...
  currplot<-ggplot(data,aes(start,log2foldchange,color=sig, label=gene_short_name))+geom_point(size=3)+scale_color_manual(values=c("black", "red"))+labs(title=paste(strain,tissue,sep=" "))+geom_text(data=subset(data, sig=='yes'))+theme_bw()+geom_vline(xintercept=0, color="blue")+geom_hline(yintercept=0,color="blue")+coord_cartesian(xlim=c(-windowSize/2, windowSize/2),ylim=c(-max(abs(data$log2foldchange),na.rm=TRUE)-1,max(abs(data$log2foldchange),na.rm=TRUE)+1))
  
  currplot<-currplot+annotate("text",x=0,y=max(abs(data$log2foldchange),na.rm=TRUE)+.5,label=paste("pvalue:",pval_for_region,sep=""))

  cisplots[[i]]<-currplot
  regions[[i]]<-genesInRegion
  nameofplot<-paste(strain,tissue,sep="_")
  ggsave(paste(nameofplot,".pdf",sep=""),plot=currplot)
}

#save(file="cisregionplotlist_exactWindows.Rdata",cisplots)
#save(file="cisregion_geneRegionslist_exactWindows.Rdata",regions)
load("cisregionplotlist_exactWindows.Rdata")
load("cisregion_geneRegionslist_exactWindows.Rdata")
```


```{r gseasetup,results='hide'}
library(xtable)
library(limma)
library(GSA)
library(gplots)
...
kegg_gs<-GSA.read.gmt("/n/rinn_data2/annotations/gsea/msigdb_v4.0_files_to_download_locally/msigdb_v4.0_GMTs/c2.cp.kegg.v4.0.symbols.gmt")

reactome_gs <- GSA.read.gmt("/Volumes/valor2/users/agroff/GSEA/c2.cp.reactome.v4.0.symbols.gmt")
biocarta_gs <- GSA.read.gmt("/Volumes/valor2/users/agroff/GSEA/c2.cp.biocarta.v4.0.symbols.gmt")
#kegg_gs<-GSA.read.gmt("/Volumes/valor2/users/agroff/GSEA/)
...
#subset by sample1/2 into a list of dfs
p<-split(population.diff,population.diff$sample_1,drop=TRUE)
annotation<-annotation(genes(cuff))

#> names(p)
#[1] "wt_brain_adult"  "wt_brain_embryo" "wt_heart"        "wt_leg"         
#[5] "wt_liver"        "wt_lung"   

doGSEA<-function(diff.pop,annotation,geneset){
  gene_names<-merge(annotation,diff.pop)
  ...
  pvl_mat <- get_gene_set_p_vals(Input.df,geneset,alternative="either")
  pvl_corrected <- get_gene_set_q_vals(pvl_mat)
  pvl_corrected<-rbind(pvl_corrected,pvl_corrected)
  zscores<-get_gene_set_ztest(Input.df,geneset)
  zscores<-cbind(zscores,zscores)
  ...
  x<-((zscores[which(colMins(pvl_corrected) < 0.01),]))
  x_ordered<-x[order(x[,1], decreasing=TRUE),]
  if(dim(x_ordered)[1]>50){x_ordered<-x_ordered[1:50,]}
...
```{r GSEAoverview}
getZscores<-function(diff.pop,annotation,geneset){
  gene_names<-merge(annotation,diff.pop)
  df.pop<-data.frame("short_name"=toupper(gene_names$gene_short_name),"test_stat"=gene_names$test_stat)
  df.pop.unique<-unique(df.pop)
  rownames(df.pop.unique)<-NULL
  ...
  pvl_corrected <- get_gene_set_q_vals(pvl_mat)
  pvl_corrected<-rbind(pvl_corrected,pvl_corrected)
  zscores<-as.data.frame(get_gene_set_ztest(Input.df,geneset))  
  #x<-((zscores[which(colMins(pvl_corrected) < 0.01),]))
  zscores$geneset<-row.names(zscores)
  zscores$zscore[which(colMins(pvl_corrected) > 0.01)]<-0
  return(zscores)
}

gseaOverviewPlot<-function(diff.popList,annotation,geneSetName){
  geneset<-get(geneSetName)
  genesetScores<-lapply(diff.popList,function(x)getZscores(x,annotation,geneset))
  zscoremat<-do.call("cbind",genesetScores)
  zscoremat<-zscoremat[,grep("geneset",names(zscoremat),invert=TRUE)]
  names(zscoremat)<-names(genesetScores)
  zscoremat_mat<-as.matrix(zscoremat)
  k <- 100
  myColors<-maPalette(low="blue",mid="white",high="red",k=k)
  myBreaks<-seq(-2,2,length.out=(k+1))
  #heatmap.2(zscoremat_mat,trace="none",col=myColors,breaks=myBreaks,margins=c(1,20),dendrogram="row",cexRow =1, offsetRow=0,main=geneSetName,labCol=colnames(zscoremat_mat),Colv="NA")
  heatmap.2(zscoremat_mat,trace="none",col=myColors,breaks=myBreaks,dendrogram="row",main=geneSetName)
}
#THIS IS WITH A heatmap.2 image, can also try ggplot tile graph 
```

```{r biocarta_gsea_overview}
pdf("biocarta_gsea_overview_contrasts.pdf")
gseaOverviewPlot(p,annotation,"biocarta_gs")
dev.off()
```

```{r kegg_gsea_overview,eval=FALSE}
#gseaOverviewPlot(p,annotation,"kegg_gs")
```

```{r reactome_gsea_overview}
pdf("reactome_gsea_overview_contrasts.pdf")
gseaOverviewPlot(p,annotation,"reactome_gs")
dev.off()
```
