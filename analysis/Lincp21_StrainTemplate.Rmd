...
dir<-"/n/rinn_data1/users/agroff/seq/OtherMice/Diana/lincP21/diff/Trp53cor1_E14.5_Leg_b0331/"
dir<-"/n/rinn_data1/users/agroff/seq/OtherMice/Diana/lincP21/diff/Trp53cor1_mike_MEFs_b0329"
#dir<-"/Volumes/Odyssey/seq/OtherMice/Diana/lincP21/diff/Trp53cor1_E14.5_Leg_b0331/"
```
...
#always_changing_gs<-GSA.read.gmt("/n/rinn_data1/users/agroff/seq/OtherMice/Diana/lincP21/analysis/always_changing_genes_lincp21.gmt")
...
makeBamTrack<-function(bamFile,bamName,genome=genome,chromosome,color="steelblue",window=20,ylim=c(0,10)){
...
pdf("lincp21_tracks_leg_10scale.pdf")
...
#gsea_file_name<-paste("GSEAinput_",timepoint,".rnk",sep="")
...
##system(paste("java -cp gsea2-2.2.0.jar -Xmx512m xtools.gsea.GseaPreranked -gmx Jacks_genesets.gmt -collapse false -mode Max_probe -norm meandiv -nperm 1000 -rnk ",gsea_file_name," -scoring_scheme weighted -rpt_label my_analysis -include_only_symbols true -make_sets true -plot_top_x 20 -rnd_seed timestamp -set_max 500 -set_min 15 -zip_report false -out GSEA_OUT/ -gui false",sep=""))
...
#consistant_change_mat<-get_gene_set_p_vals(Input.df,always_changing_gs#,alternative="either")
#consistant_corrected<-get_gene_set_q_vals(consistant_change_mat)
#consistant_corrected<-rbind(consistant_corrected,consistant_corrected)
...
#consistant_zscores<-get_gene_set_ztest(Input.df,always_changing_gs)
#consistant_zscores<-cbind(consistant_zscores,consistant_zscores)
...
Jacks enrichment
```{r jacks_enrich, fig.width=10,fig.height=12}
InputCols<-maPalette(low="white",high="red",k=100)
...
```{r jacks_zscore, fig.width=10,fig.heigh=12}
...
```{r consistant_enrich, eval=FALSE, fig.width=10,fig.height=12}
InputCols<-maPalette(low="white",high="red",k=100)
x<-(-log10(t(consistant_corrected[,which(colMins(consistant_corrected) < 0.001)])))
x_ordered<-x[order(x[,1], decreasing=TRUE),]
noinfinities_x<-x_ordered[which(x_ordered !="Inf")]
x_max<-max(noinfinities_x)+100
x_ordered[x_ordered=="Inf"]<-x_max
...
```{r consistant_zscore, eval=FALSE,fig.width=10,fig.heigh=12}
x<-consistant_zscores[which(colMins(consistant_corrected) < 0.001),]
x_ordered<-x[order(x[,1], decreasing=TRUE),]
x_ordered<-as.matrix(x_ordered)
...
```{r GSEA_all,eval=FALSE, fig.width=10, fig.height=16}
InputCols<-maPalette(low="white",high="red",k=100)
...
```{r gsea_enrich_fig,eval=FALSE, fig.width=8, fig.height=16}
...
```{r gsea_zscore_fig,eval=FALSE, fig.width=8, fig.height=16}
#ALL zscore: 
...
Binomial test for number of genes DE in region compared to DE genes in whole transcriptome

```{r binomialTestDE}
binom.test(nSig,26754,length(getSig(cuff))/26754)
#number DE genes in region, number genes total, degenes_total/numGenestotal
```

hypergeometric sig genes in region :: (probability of more than nSig num genes DE in this region from this transcriptome)


```{r} hypergeoSigGenesRegion}
nDE_tissue<-length(getSig(cuff))
hyper_pval<-phyper(nSig,nDE_tissue,26754-nDE_tissue,nGenes_lncRNA,lower.tail=FALSE)
hyper_pval
```

# Other clusters of DE genes

```{r otherclustersDEgenescalc,results='hide'}

sigAnnot<-annotation(sigGenes)
firstSplit<-str_split_fixed(sigAnnot$locus,":",2)
secondSplit<-str_split_fixed(firstSplit[,2],"-",2)
sigAnnot$chromosome<-firstSplit[,1]
sigAnnot$start<-as.numeric(secondSplit[,1])
sigAnnot$end<-as.numeric(secondSplit[,2])
sigAnnot<-sigAnnot[order(sigAnnot$locus),]

#find 4Mb regions w nSig or more DE genes and make a dataframe 
#sum number of entries per chromosome
splitbyChr<-split(sigAnnot,sigAnnot$chromosome)
moreGenes<-lapply(seq(1,length(splitbyChr)),function(x)dim(splitbyChr[[x]])[1])
moreGenes<-unlist(moreGenes)
chromsToCheck<-splitbyChr[which(moreGenes>=nSig)]
#if less than nSig, ignore that chromosome 

findWindowsInChrDF<-function(chrDF){
  #for remaining chr, step through each start and count #DE in 4Mb window 
    # if >=nSig DE genes between start and start+window
      # window=coords
      # else window=no
    # if start+window>last end, stop and go to next chr 
  window<-4000000
  windows<-apply(chrDF,1,function(x){
    start<-as.numeric(x[9])
    end<-start+as.numeric(window)
    num<-length(chrDF[which(chrDF$start>=start & chrDF$start<=end),1])
    if(num>=nSig){
      write(paste(chrDF[which(chrDF$start>=start & chrDF$start<=end),4],collapse="_"),stderr())
      result<-paste(paste(x[8],start,sep=":"),end,sep="-")
    }else{
      result<-"no sig window"
    } 
    
    result
  })
  chrDF$windows<-windows
  chrDF  
  }

sigWindows<-lapply(seq(1,length(chromsToCheck)),function(x)findWindowsInChrDF(chromsToCheck[[x]]))

sigWindows<-do.call(rbind.data.frame,sigWindows)
#do.call pass elements of your_list as arguments to rbind. 
#http://stackoverflow.com/questions/4227223/r-list-to-data-frame
sigWindows<-sigWindows[which(sigWindows$windows!="no sig window"),]

colsToKeep<-c("gene_id","gene_short_name","locus","windows")
sigWindows<-sigWindows[,colsToKeep]
row.names(sigWindows)<-NULL

```

```{r otherDEclusterTABLE,results='asis'}
print(xtable(sigWindows),type="html")
```
